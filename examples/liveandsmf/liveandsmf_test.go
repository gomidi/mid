...
func (e *example) Tempo(p mid.SMFPosition, bpm uint32) {
	e.bpm = bpm
}

// NoteOn responds to note on messages
func (e *example) NoteOn(p *mid.SMFPosition, channel, key, vel uint8) {
	fmt.Printf("[%vs] NoteOn at channel %v: key %v velocity: %v\n",
		e.calcDuration(p).Seconds(), channel, key, vel)
}

func (e *example) NoteOff(p *mid.SMFPosition, channel, key, vel uint8) {
...
func (e *example) calcDuration(p *mid.SMFPosition) (dur time.Duration) {
...
func Example() {
	...
	rd := mid.NewReader(mid.NoLogger() /* disable default logging*/)
